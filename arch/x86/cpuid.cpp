...
cpuid::CPU_Vendor cpuid::get_vendor()
{
    ...
    auto answer = cpu_vendor_string_table.search([buffer](CPU_String_Couple search_term) {
        const auto *array_tmp = reinterpret_cast<const Array<char, 12> *>(search_term.cpu_vendor_string.raw());
        return *array_tmp == buffer;
    });

    return (answer.valid) ? cpu_vendor_string_table[answer.data].cpu_vendor : CPU_Vendor::UNKNOWN;
}
